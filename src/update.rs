...
pub fn update_wh<T>(
    k: usize, l: usize, lambda: T,
    w: &mut Array3<T>, h: &mut Array2<T>, x_bar: &mut Array2<T>, wtx: &mut Array2<T>,
    x: &Array2<T>, x_pad: &Padded2D<T>,
    cost: &mut Vec<T>, epsilon: T,
    update_w: bool, update_h: bool,
) 
where
    T: Float + std::ops::AddAssign + ScalarOperand + std::ops::MulAssign + 'static
{
    ...
    if update_w {
        for i in 0..l {
            let hlt = h_pad.shift_view(i, ShiftDirect::Right).unwrap().t().to_owned();
            let n0 = x.dot(&hlt);
            let d0 = x_bar.dot(&hlt);
            let d1 = x_pad.shift_view(i, ShiftDirect::Right).unwrap().dot(&l_sh_one_eye);
            let dw1 = n0 / (d0 + d1 + epsilon);
            Zip::from(&mut w_new.slice_mut(s![.., .., i])).and(&dw1).apply(|a, &b| {
                *a *= b;
            });
        }
    }
