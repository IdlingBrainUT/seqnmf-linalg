...
impl<T: Float> Padded2D<T> {
    pub fn from_arr(a: &Array2<T>, padding_size: usize) -> Self {
        ...
        Self { data, raw_size, padding_size }
    }
    ...
    pub fn shift_view(&self, l: usize, shift_dir: ShiftDirect) -> Result<ArrayView2<T>, &str> {
        ...
        if padding_size < l { return Err("Shitf size l is larger than padding size!"); }
    ...
    T: Float + std::ops::AddAssign + std::ops::MulAssign + 'static
{
    fn convolution(&self, rhs: &Padded2D<T>) -> Array2<T> {
        ...
        for i in 0..l {
            let brr = self.slice(s![.., .., i]).dot(&rhs.shift_view(i, ShiftDirect::Right).unwrap());
    ...
    fn transpose_convolution(&self, rhs: &Padded2D<T>) -> Array2<T> {
        ...
        for i in 0..l {
            let brr = self.slice(s![.., .., i]).t().dot(&rhs.shift_view(i, ShiftDirect::Left).unwrap());
            Zip::from(&mut arr).and(&brr).apply(|a, &b| {
                *a += b;
            });
        }
    ...
    T: Float + Clone
{
    fn smooth(&self, l: usize) -> Self {
        ...
        for i in 0..cols {
            ...
            Zip::from(&mut arr.slice_mut(s![.., i])).and(&brr).apply(|a, &b| {
                *a = b;
            });
        }
        arr
...
impl<T: Float + SampleUniform> RandomArray for Array2<T> {
    ...
    fn shuffle(&self, random_seed: u8) -> Self {
        ...
        for y in 0..y_size {
            ...
            Zip::from(&mut arr.slice_mut(s![y, i..])).and(&self.slice(s![y, ..x_size-i])).apply(|a, &b| {
                *a = b;
            });
            Zip::from(&mut arr.slice_mut(s![y, ..i])).and(&self.slice(s![y, x_size-i..])).apply(|a, &b| {
                *a = b;
...
impl<T: Float + SampleUniform> RandomArray for Array3<T> {
    ...
    fn shuffle(&self, random_seed: u8) -> Self {
        ...
        for ki in 0..k {
            for ni in 0..n {
                let i = ud.sample(&mut rng);
                Zip::from(&mut arr.slice_mut(s![ni, ki, i..])).and(&self.slice(s![ni, ki, ..l-i])).apply(|a, &b| {
                    *a = b;
                });
                Zip::from(&mut arr.slice_mut(s![ni, ki, ..i])).and(&self.slice(s![ni, ki, l-i..])).apply(|a, &b| {
                    *a = b;
